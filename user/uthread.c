...
thread_create(void (*func)())
{
  ...
  t->state = RUNNABLE;
  // when thread_scheduler() runs a given thread for the first time
  // the thread has to executes function `func`
  t->context.ra = (uint64)func;
  // on its own stack. 
  // Here register sp points to the bottom of stack
  // for the reason that stack grows downwards.
